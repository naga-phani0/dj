...
class TCS:
            ...
            try:
                ...
                if url==j_url[:-1]:
                    ...
                    try:
                        ...
                        job_title=""
                    try:
                        impinfo="Job Expires By : "+j_data['applyby']+"\nAdditional info : "+str(j_data['additionalInfo'])
                    except:
                        impinfo=""
                    try:
                        self.threadlock.acquire()
                        ...
                        self.objct.upload_job_meta_upd(postauth=POST_AUTHOR, postcontent=job_desc, posttitle=job_title, companyname='TCS',
                                                    location=job_loc, jobtype=job_type,search_page_no=p_url,job_url=j_url,
                                                    qualification=job_qual, skills=job_skills, experience=job_exp,
                                                    imp_info=impinfo,company_website="https://www.tcs.com/", 
                                                    company_tagline="We're Building On Belief", company_video="Not Available", company_twitter="@TCS",
                                                    job_logo=True,
    ...
    def multi_thread_updated(self):
        ...
        try:
            ...
            self.logger_ob.critical(f'Error while inserting links in status table using multithreading : {st_tb_mul_thd_err}')
            ...
            try:
                ...
                self.logger_ob.critical(f'Error while inserting links in trial_job_meta using multithreading : {job_scp_mt_err}')
                self.objct.exit_fun()
